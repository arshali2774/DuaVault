...
import {
  ...
  type DuplicateMatch,
} from "@/components/duplicate-warning-modal";
...
export function DuaForm({
  ...
  const submitDua = async (confirmDuplicate: boolean) => {
    ...
    try {
      ...
      if (response.status === 409) {
        const { possibleDuplicate } = await response.json();
